...
extern uint16_t tss_get_selector_check(void);

#include "core/usermode.h"
#include "syscall.h"
static uint8_t user_stack[4096] __attribute__((aligned(4096)));
static uint8_t syscall_kernel_stack[4096] __attribute__((aligned(4096)));

static uint8_t user_stack[4096] __attribute__((aligned(4096)));

void __attribute__((noinline)) usermode_test_func(void) {
    const char* msg = "hello from ring3\n";
    int ret;
    __asm__ __volatile__(
        "int $0x80"
        : "=a"(ret)
        : "a"(SYS_WRITE), "b"(1), "c"(msg), "d"(17)
        : "memory"
    );
    for (;;) { }   // never hlt here - hlt is privileged, would fault in ring 3
}

static void cmd_ring3(const char *args) {
    (void)args;
    vga_puts("dropping to ring 3...\n");

    extern void vmm_mark_user_range(void*, size_t);
    vmm_mark_user_range((void*)usermode_test_func, 4096);
    vmm_mark_user_range(user_stack, sizeof(user_stack));

    extern void tss_set_kernel_stack(uint32_t);
tss_set_kernel_stack((uint32_t)(syscall_kernel_stack + sizeof(syscall_kernel_stack)));

    enter_usermode((void*)usermode_test_func, user_stack + sizeof(user_stack));

    vga_puts("this line should never print\n"); // control never returns here
}

static void cmd_tss(const char *args) {
    (void)args;
    vga_puts("GDT/TSS check\n");
    ...
    vga_puts("CS = "); vga_puthex(cs); vga_puts("\n");
    vga_puts("DS = "); vga_puthex(ds); vga_puts("\n");
    vga_puts("TR (task register) = "); vga_puthex(tr); vga_puts("\n");
}

static void cmd_syscall(const char *args) {
    (void)args;
    vga_puts("syscall test - calling sys_write via int 0x80\n");
    ...
    __asm__ __volatile__(
        ...
        : "a"(SYS_WRITE), "b"(1), "c"(msg), "d"(19)   // eax=syscall#, ebx=fd, ecx=buf, edx=len
        : "memory"
    );

    vga_puts("sys_write returned: ");
    vga_puthex((uint32_t)ret);
    vga_puts("\n");
}
...
void kernel_main(void) {
    ...
    shell_register_command("syscall", cmd_syscall, "Test syscall path");
    shell_register_command("tss", cmd_tss, "Check GDT/TSS state");
    shell_register_command("ring3", cmd_ring3, "Drop to ring 3 and test syscall");
