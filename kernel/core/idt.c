...
static void serial_hex(uint32_t val) {
    ...
    for (int i = 0; i < 8; i++)
        buf[2 + i] = "0123456789ABCDEF"[(val >> ((7 - i) * 4)) & 0xF];
    hal_serial_write(buf, 10);
}
...
void exception_handler(registers_t* regs) {
    ...
    if (regs->int_no < 19) {
        ...
        hal_serial_write(name, strlen(name));
    }
    ...
    if (regs->int_no == 14) { // Page fault
        ...
        hal_serial_write("\nCause: ", 8);
        hal_serial_write((regs->err_code & 0x1) ? "protection-violation" : "not-present", (regs->err_code & 0x1) ? 21 : 12);
        hal_serial_write(", ", 2);
        hal_serial_write((regs->err_code & 0x2) ? "write" : "read", (regs->err_code & 0x2) ? 5 : 4);
        hal_serial_write(", ", 2);
        hal_serial_write((regs->err_code & 0x4) ? "user-mode" : "kernel-mode", (regs->err_code & 0x4) ? 9 : 11);
    }
