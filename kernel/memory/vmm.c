...
static uint32_t kernel_page_table[20][1024] __attribute__((aligned(4096)));
...
void vmm_init(void) {
    ...
    for (uint32_t i = 0; i < 80 * 1024 * 1024 / 0x1000; i++) { // 8MB / 4KB
        uint32_t phys_addr = i * 0x1000;
        // Set the page table entry: present, writable, kernel mode
        kernel_page_table[i / 1024][i % 1024] = phys_addr | 0x001 | 0x002;
    }
    ...
    for (int i = 0; i < 20; i++) {
        kernel_page_directory[i] = pt_addr | 0x001 | 0x002;
    ...
    hal_serial_write("STAGE2b-pre-enable-paging\n", 26);
    vmm_switch_directory(kernel_page_directory);
    hal_serial_write("STAGE2b-dir-switched\n", 21);
    vmm_enable_paging();
    hal_serial_write("STAGE2b-paging-enabled\n", 23);
}

// Create a brand new, empty memory map for a process
uint32_t* vmm_create_address_space(void) {
    ...
    uint32_t *new_dir = (uint32_t*)phys;
    for (int i = 0; i < 1024; i++) {
        new_dir[i] = 0; // wipe it clean
    }

    // Copy the kernel's own mappings so the process can still call kernel code
    for (int i = 0; i < 20; i++) {
...
int vmm_map_page(void* virt_addr, uint32_t phys_addr, uint32_t flags) {
    ...
    pt[page_table_index] = (phys_addr & 0xFFFFF000) | (flags & 0xFFF) | PAGE_PRESENT;
...
// Flip PAGE_USER on already-mapped pages covering [start, start+len).
// Used for marking specific kernel-resident code/stack as ring-3 accessible for testing.
void vmm_mark_user_range(void* start, size_t len) {
    uint32_t addr = (uint32_t)start & ~0xFFF;
    uint32_t end = ((uint32_t)start + len + 0xFFF) & ~0xFFF;
    uint32_t* dir = vmm_get_current_directory();

    for (; addr < end; addr += 0x1000) {
        uint32_t pdi = addr >> 22;
        uint32_t pti = (addr >> 12) & 0x3FF;
        uint32_t pde = dir[pdi];
        if (!(pde & PAGE_PRESENT)) continue;
        dir[pdi] |= PAGE_USER;
        uint32_t* pt = (uint32_t*)(pde & 0xFFFFF000);
        pt[pti] |= PAGE_USER;
        vmm_flush_tlb_entry((void*)addr);   // <-- new: force the CPU to forget the stale cached entry
    }
}
