...
typedef struct process {
    ...
    void *stack_ptr;    
    uint32_t stack_phys;              /* Physical address of this task's stack page, for fork/cleanup */              /* Stack pointer for context switching */
    uint32_t *page_directory;         /* This process's own memory map (NULL = uses kernel's) */
    void *entry_point;                /* Entry point of the process */
    void *arg;                        /* Argument passed to entry point */
    uint32_t wake_tick;                /* Tick count at which this task wakes up */
