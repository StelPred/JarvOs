...
int sys_fork(void) {
    ...
    process_t *child = scheduler_fork_process(parent);
    if (child == NULL) {
        return -1; // fork failed (out of memory/PCBs)
    }
...
int sys_exit(int status) {
    ...
    if (proc != NULL) {
        ...
        scheduler_destroy_process(proc);   // release PCB back to free_list
    }
    scheduler_yield();                     // give up the CPU now
    for (;;) { __asm__ __volatile__("hlt"); } // safety net, should never reach here
    return 0;
}
