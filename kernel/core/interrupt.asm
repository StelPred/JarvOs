...
%macro ISR_NOERRCODE 1
  global isr%1
  isr%1:
    push dword 0          ; dummy error code (this exception has none)
    push dword %1         ; interrupt number, so C knows which exception fired
    pusha
    push ds
    push es
    push fs
    ...
    mov fs, ax
    mov gs, ax
    push esp               ; pass pointer to all saved data as the argument
    ...
    pop ds
    popa
    add esp, 8               ; remove int_no + error code
    iretd
%endmacro
...
%macro ISR_ERRCODE 1
  global isr%1
  isr%1:
    ; CPU already pushed a real error code, no dummy needed
